    ...
    <div className={`dashboard-header h-16 shrink-0 bg-white border-b border-gray-100 px-6 flex items-center justify-between sticky top-0 z-50 ${styles.dashboardHeader}`}>
        ...
        >
          <div className="relative w-full flex items-center justify-center">
            ...
            />
            {showSearchBar && (
              <button type="button" onClick={() => setShowSearchBar(false)} className="md:hidden absolute right-7 text-gray-400">
                <X size={20} />
              </button>
            )}
          </div>
          <button type="submit" className="hidden md:flex bg-[var(--budgeet-primary-light)] w-12 h-10 justify-center items-center rounded-r-lg border border-l-0 border-gray-200 hover:bg-[var(--budgeet-primary)] hover:text-white transition-colors">
            <Search size={20} />
          </button>
        </form>
        

      <div className={`left h-full flex items-center gap-4 ${showSearchBar ? 'hidden md:flex' : 'flex'}`}>
        <Bell size={20} className="text-gray-500 cursor-pointer hover:text-black transition-colors" />
